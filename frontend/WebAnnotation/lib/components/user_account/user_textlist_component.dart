...
class UserTextlistComponent implements OnInit {
  ...
  Future<Null> ngOnInit() async {
    if(userAccountService.loggedIn) {
      userAccountService.queryTexts().then((success) {
        if(!success) {
          appService.errorMessage(switchLang("Abrufen der User Texte fehlgeschlagen."));
        }
      });
    }
  }

  List<UserText> getUsertext() {
    if(userAccountService.loggedIn) {
      userAccountService.queryTexts().then((success) {
        if (!success) {
          appService.errorMessage(
              switchLang("Abrufen der User Texte fehlgeschlagen."));
        }
      });
  }}
  ...
  String switchLang(String s){
    if (langService.isEng){
      window.history.pushState('en','en','user-textlist/en');
    } else {
      window.history.pushState('de','de','user-textlist/de');
    }
