...
from DictionaryService import DictionaryService, Segmentation, DictionaryServiceen
...
@app.route('/query/texten', methods=['POST'])
def query_texten():
    user = userService.authenticate(Authentication.read(request))
    text = request.form.get("text")
...
@app.route('/query/segmentationen/<word>', methods=['GET'])
def query_segmentationen(word):
    response = dictionaryServiceen.query_segmentation(word)
...
@app.route('/query/def', methods=['POST'])
def query_def():
    text = request.form.get("text")
    lemma = request.form.get("lemma")
    pos = request.form.get("pos")

    response = dictionaryService.find_def(text, lemma, pos)

    if response is None:
        return create_error_response(404, "Error finding the definition.")

    return create_response(200, response)        
    
    
@app.route('/query/defen', methods=['POST'])
def query_defen():
    text = request.form.get("text")
    lemma = request.form.get("lemma")
    pos = request.form.get("pos")

    response = dictionaryServiceen.find_def(text, lemma, pos)
...
@app.route('/user/verificationen/query', methods=['POST'])
def query_verificationen():
    ...
    if verification_proposals is not None:
        for p in verification_proposals:
            s = Segmentation(word['text'], p.user.first_name + " " + p.user.last_name, "User", p.hyphenation, p.stress_pattern)
            segmentations.append(s.json())

    segmentation_proposals = dictionaryServiceen.query_segmentation(word['text'])
    segmentations = segmentations + segmentation_proposals
...
@app.route('/user/verificationen/submit', methods=['POST'])
def submit_verificationen():
    ...
    if not verificationService.submit(user, word_id, stress_pattern, hyphenation, dictionaryServiceen):
        return create_error_response(404, "Unable to submit proposal.")
