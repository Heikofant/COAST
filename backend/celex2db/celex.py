...
class CelexDictionary_en:
    ...
    def parse_lemmas(self, max_lines, step_size):
        # open celex2 eml
        epwFile = open("./celex2/english/eml/eml.cd")
        ...
        while True:
            ...
            if len(components) < 22:
                # word class should be the 22th part
                continue

            lemma_id = components[0]
            text = components[1]
            word_class = components[21]

            pos_tag = 'na'
			# find the second last letter of 22th component (word_class) 
            index_end = word_class.rfind(']')
            if index_end < 2:
                numErrors = numErrors + 1
            else:
    ...
    def parse_words(self, max_lines, step_size):
        # open celex2 file (epw.cd)
        epwFile = open("./celex2/english/epw/epw.cd")
        eowFile = open("./celex2/english/eow/eow.cd")
        ...
        while True:
            lineEpw = epwFile.readline()
            lineEow = eowFile.readline()
            if not lineEpw or not lineEow:
                break
            ...
            text, word_id, lemma, phon, pos_tag = self.parsePhoneticEntry(lineEpw)
            if not text:
                numErrors = numErrors + 1
                continue

            hyphenation = self.parseOrthograficEntry(lineEow, word_id)
            if not hyphenation:
                numErrors = numErrors + 1
                continue
    ...
    def parsePhoneticEntry(self, line):
        components = line.split('\\')
        if len(components) < 7:
            # phonetic pattern should be the 7th part
            return None
    ...
    def parseOrthograficEntry(self, line, word_id_phon):
        components = line.split('\\')
        if len(components) < 9:
            # phonetic pattern should be the 9th part
            return None
