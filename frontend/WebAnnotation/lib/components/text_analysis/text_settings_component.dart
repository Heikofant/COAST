...
class TextSettingsComponent implements OnInit, TextAnalysisObserver {
  ...
  List<Option> foregroundOptions = [
    new Option("Schriftfarbe", false, false),
  ...
  void saveConfiguration() {
    if(selectedConfiguration().name == newConfigurationName) {
      userAccountService.updateTextConfiguration(selectedConfiguration()).then((success) {
        if(!success) {
          appService.errorMessage(switchLang("Vorlage konnte nicht gespeichert werden."));
        } else {
          appService.infoMessage(switchLang("Vorlage gespeichert."));
      ...
      userAccountService.addTextConfiguration(selectedConfiguration()).then((success) {
        if(!success) {
          appService.errorMessage(switchLang("Neue Vorlage konnte nicht angelegt werden."));
        } else {
          appService.infoMessage(switchLang("Neue Vorlage angelegt."));
  ...
  void deleteConfiguration(TextConfiguration configuration) {
    userAccountService.deleteTextConfiguration(configuration).then((success) {
      if(!success) {
        appService.errorMessage(switchLang("Vorlage konnte nicht gelöscht werden."));
      } else {
        appService.infoMessage(switchLang("Vorlage gelöscht."));
