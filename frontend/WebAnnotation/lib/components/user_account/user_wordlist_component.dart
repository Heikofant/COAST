...
class UserWordlistComponent implements OnInit {
  ...
  Future<Null> ngOnInit() async {
    if(userAccountService.loggedIn) {
      userAccountService.queryWords().then((success) {
        if(!success) {
          appService.errorMessage(switchLang("Abrufen der User Wörter fehlgeschlagen."));
        }
      });
    }
  }
    ...
    userAccountService.deleteWord(userWord).then((success) {
      if(!success) {
        if(langService.isEng){
          appService.errorMessage("Deleting the word " + userWord.text +" failed.");
        } else {
  ...
  String switchLang(String s){
    if (langService.isEng){
      window.history.pushState('en','en','user-wordlist/en');
    } else {
      window.history.pushState('de','de','user-wordlist/de');
    }
