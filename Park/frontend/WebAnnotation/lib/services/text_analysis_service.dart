...
class TextAnalysisService {
  ...
  TextAnalysisService(this._santizationService);
  ...
  TextConfiguration selectedConfiguration =   new TextConfiguration(-1, "",
      '#0000A0', '#FF0000', '#FFFFFF', '#008006 ', '|', 16.0, 1.5, 0.2, 0.4, 0.0,
      false, true, true, true, true, new PartOfSpeechConfiguration());

  /*
  new TextConfiguration(-1, "",
  ...
  Future<bool> lookup(var userData) async {
    ...
    if(langService.isEng){
      url = AppService.SERVER_URL + "/query/texten";
    } else{
      url = AppService.SERVER_URL + "/query/text";
    }
  ...
  SafeUrl sanitize(toSanitize){
    return _santizationService.bypassSecurityTrustUrl(toSanitize);
  }
  ...
  Future<bool> dictionary(String word, String lemma, String pos) {
    String url;
    if(langService.isEng) {
      url = AppService.SERVER_URL + "/query/defen";
    } else {
      url = AppService.SERVER_URL + "/query/def";
    }
