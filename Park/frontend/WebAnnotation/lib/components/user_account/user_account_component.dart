...
class UserAccountComponent implements OnInit {
  ...
  final UserDeleteComponent userDeleteComponent;
  final Router router;

  UserAccountComponent(this.appService, this.userAccountService, this.textAnalysisService, this.userDeleteComponent, this.router);
  ...
  void logout() {
    ...
    appService.infoMessage(switchLang("Logout erfolgreich."));
  }

  void deleteAccount() {
    router.navigate(['DeleteUser']);
  }

  String switchLang(String s){
    if (langService.isEng){
      window.history.pushState('en','en','user-account/en');
    } else {
      window.history.pushState('de','de','user-account/de');
    }
