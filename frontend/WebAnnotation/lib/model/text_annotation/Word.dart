  ...
  bool parseHyphenationUsingOriginalText(String hyphenation) {
    ...
    for (int i = 0; i < syllables.length; i++) {
      ...
      if (syllable.toLowerCase() != lookupSyllable) {
        ...
        // works only alright for german, doesn't work for English words with only one syllable
        // For example, when tested with 'Those' and 'those', although it's one syllable
        // it directly switches to 'those' and doesn't keep the original text (capitals)
        if (!langService.isEng) {
