...
class WordVerificationComponent implements OnInit {
  ...
  String userLevel() {
    ...
    if(userAccountService.isExpert) {
      return switchLang("Experte");
    }

    return switchLang("Benutzer");
  }
  ...
  void submitWord() {
    ...
    segmentationVerificationService.submitVerification(userAccountService.credentials(),
      ...
      if(!success) {
        appService.errorMessage(switchLang("Vorschlag konnte nicht gesendet werden."));
      }
  ...
  String switchLang(String s){
    if (langService.isEng){
      window.history.pushState('en','en','word-verification/en');
    } else {
      window.history.pushState('de','de','word-verification/de');
    }
